            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              <Card className="bg-gradient-to-br from-primary to-accent text-primary-foreground">
                <CardContent className="pt-6">
                  ...
                  <p className="font-medium">{t("courseOverview.overallProgress")}</p>
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <p className="text-muted-foreground font-medium">{t("courseOverview.completedTopics")}</p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <p className="text-muted-foreground font-medium">{t("courseOverview.totalChapters")}</p>
                </CardContent>
              </Card>
            </div>
                          ...
                          <div className="flex-1 w-full">
                            {chapter.quiz && !quizPassed && (
                                <div className="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg w-fit mb-2">
                                  ...
                                  <span>{t("courseOverview.quizRequired")}</span>
                                </div>
                            )}
                          ...
                          <div className="w-full sm:w-auto">
                            {!chapterAccessible ? (
                                <Button variant="secondary" disabled size="sm" className="w-full sm:w-auto">
                                  <Lock className="w-4 h-4 mr-2" /> {t("courseOverview.locked")}
                                </Button>
                            ) : showQuiz ? (
                                <Link to={`/course/${course.id}/chapter/${chapter.id}/quiz`}>
                                  <Button size="sm" className="w-full sm:w-auto">
                                    <ClipboardCheck className="w-4 h-4 mr-2" /> {t("courseOverview.startTest")}
                                  </Button>
                                </Link>
                            ) : firstTopic ? (
                                <Link to={`/course/${course.id}/chapter/${chapter.id}/topic/${firstTopic}`}>
                                  <Button size="sm" className="w-full sm:w-auto">
                                    {chapterProgress > 0 ? t("courseOverview.continue") : t("courseOverview.start")}
