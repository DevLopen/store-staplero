...
export const getCourseById = async (req: Request, res: Response) => {
    try {
        console.log("Fetching courseId:", req.params.courseId);
...
export const updateChapterStatuses = async (courseId: string, userProgress: Record<string, boolean>, userQuizResults: Record<string, {passed: boolean}>) => {
    const course = await Course.findById(courseId);
    if (!course) return;

    course.chapters.forEach((ch, idx) => {
        const allTopicsDone = ch.topics.every(t => userProgress[t.id]);
        const quizPassed = ch.quiz ? userQuizResults[ch.id]?.passed : true;

        if (idx > 0) {
            const prev = course.chapters[idx-1];
            const prevDone = prev.quiz ? userQuizResults[prev.id]?.passed : prev.topics.every(t => userProgress[t.id]);
            if (!prevDone) {
                ch.status = "blocked";
                return;
            }
        ...
        if (allTopicsDone && (!ch.quiz || quizPassed)) {
            ch.status = "complete";
